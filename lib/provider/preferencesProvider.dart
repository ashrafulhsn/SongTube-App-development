...
class PreferencesProvider extends ChangeNotifier {
  ...
  bool get youtubeAutoPlay {
    return prefs.getBool('youtubeAutoPlay') ?? false;
  }
  ...
  set watchHistory(List<StreamInfoItem> history) {
    List<Map<String, dynamic>> map = history.map((e) => e.toMap()).toList();
  ...
  void streamPlaylistCreate(
      String playlistName, String author, List<StreamInfoItem> streams) {
    StreamPlaylist playlist = StreamPlaylist(
        name: playlistName, author: author, streams: streams, favorited: false);
  ...
  bool streamPlaylistHasStream(String playlistName, StreamInfoItem stream) {
    List<StreamPlaylist> playlists = streamPlaylists;
    int playlistIndex =
        playlists.indexWhere((element) => element.name == playlistName);
    if (playlistIndex == -1) return false;
    StreamPlaylist playlist = playlists[playlistIndex];
    int streamIndex =
        playlist.streams.indexWhere((element) => element.id == stream.id);
    if (streamIndex == -1) {
      return false;
    } else {
      return true;
  ...
  set channelSubscriptions(List<ChannelSubscription> subscriptions) {
    prefs.setString(
        'subscriptions', ChannelSubscription.toJsonList(subscriptions));
    notifyListeners();
  }
  ...
  void enableChannelNotifications(String channelUrl) {
    var subscriptions = channelSubscriptions;
    int index =
        subscriptions.indexWhere((element) => element.url == channelUrl);
    subscriptions[index].enableNotifications =
        !subscriptions[index].enableNotifications;
    channelSubscriptions = subscriptions;
  }
